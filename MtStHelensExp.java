...
public class MtStHelensExp implements AIModule
{
    public List<Point> createPath(final TerrainMap map) 
    {
        ...
        {//loop while the openList is not empty
            ...
            for(Point v : neighbors)
            {
                ...
                {//either in closedlist or never explored

                    if(closed.containsKey(v)) 
                    {//we need this in case the heuristic is not consistent
    ...
    private double getHeuristic(final TerrainMap map, final Point pt1, final Point pt2)
    {
        ...
        double deltaH = Math.abs(h1-h2);

        if (h1 < h2)
        {//we have to ascend to get to the goal
